...
def _eval(cat=None, start=None, end=None, baseline=None, response_id="r1", ctx=None, **kw):
    cat = cat or _cat()
    start = start or _ep(100.0, _dt(18, 0, 0), _dt(18, 0, 30))
    end = end or _ep(102.0, _dt(18, 7, 0), _dt(18, 7, 30))
    return C.evaluate_catalyst_response(ctx or _ctx(), cat, start, end, baseline=baseline, response_id=response_id, **kw)


# ── schema ──
def test_schema():
    assert C.V2_CATALYST_RESPONSE_SCHEMA_VERSION == "2F.1"
...
def test_baseline_fit_after_catalyst_blocks():
    a = _eval(baseline=_baseline(fit_window_end=_dt(17, 21, 0)))  # after catalyst event 20:00
    assert a.association_status == "BLOCKED"
...
def _assessments():
    ...
    a1 = _eval(ctx=ctx, response_id="h1", end=_ep(102.0, _dt(18, 7, 0), _dt(18, 7, 30)))  # +2%
    a2 = _eval(ctx=ctx, response_id="h2", end=_ep(101.2, _dt(19, 7, 0), _dt(19, 7, 30)))  # +1.2%
    a3 = _eval(ctx=ctx, response_id="h3", end=_ep(100.8, _dt(20, 7, 0), _dt(20, 7, 30)))  # +0.8%
    return [a1, a2, a3]
...
def test_response_path_requires_same_catalyst():
    ...
    assert p.path_id == ""  # mismatch → empty path
