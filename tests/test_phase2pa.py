...
def test_publication_manifests_exist():
    assert (ROOT / "research/phase2/publication/PUBLICATION_FILE_MANIFEST.txt").exists()
    assert (ROOT / "research/phase2/publication/PUBLICATION_EXCLUDE_MANIFEST.txt").exists()

def test_release_manifest_truthful():
    m = _yaml("research/releases/v2.0.0-rc1/RELEASE_MANIFEST.yaml")
...
def test_research_freeze_truthful():
    m = _yaml("research/phase2/freeze/PHASE2_RESEARCH_FREEZE.yaml")
...
def test_crash_recovery_reports_exist():
    assert (ROOT / "research/phase2/publication/PHASE2PA_CRASH_RECOVERY_AUDIT.md").exists()
    assert (ROOT / "research/phase2/publication/PHASE2PA_CRASH_RECOVERY_REPORT.md").exists()
    assert (ROOT / "research/phase2/publication/PHASE2PA_FINAL_PUBLICATION_ACCEPTANCE_REPORT.md").exists()

def test_acceptance_gate_pass():
    assert "PHASE2PA_PASS" in _doc("research/phase2/publication/PHASE2PA_FINAL_PUBLICATION_ACCEPTANCE_REPORT.md")

def test_git_plan_forbids_force_push():
    p = _doc("research/phase2/publication/PRE_PUBLICATION_GIT_PLAN.md")
