...
def main() -> int:
    ...
    # 3. synthetic forecast + 4. quantiles 0.1/0.5/0.9（per-step path，V1 remediation）
    try:
        adapter = cpu
        pred = adapter.predict(synthetic, horizon=7, quantiles=[0.1, 0.5, 0.9])
        path = pred["path"]
        qs = sorted(path.keys())
        ok = qs == ["p10", "p50", "p90"] and len(path["p50"]) == 7
        results["forecast_quantiles"] = "PASS" if ok else f"FAIL: {pred}"
        print("[3][4] forecast + quantile path:", "PASS" if ok else "FAIL", {q: path[q][:3] for q in qs})
    except Exception as e:
