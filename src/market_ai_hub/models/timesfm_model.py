"""TimesFM 3.0 adapter（spec §6 + V1 remediation：多步 forecast path）。
...
V1 remediation 修正：
- predict 回傳 per-step path（每步 p10/p50/p90），不再 horizon 平均。
- horizon 參數真實傳到底層 horizon。
"""
...
class TimesFM3Adapter:
    ...
    def predict(
        ...
        path = {f"p{int(q * 100)}": [float(v) for v in qmat[:, idx[q]]] for q in quantiles}
        return {"path": path}
...
def timesfm_forecast(
    ...
    warnings = [LICENSE_TAG] if q_valid else [LICENSE_TAG, f"quantile contract violated: {q_reason}"]
