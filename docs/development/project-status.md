...
- Current phase: **C2 W3.3 REAL RAW EVIDENCE CAPTURED / C2.3 OFFLINE_PASS / RUNTIME_REVERIFICATION_PENDING / SAFE_DEFAULT_RECORDER_RUNNING**
...
- Live quote capability：2026-09-24 local quote matrix reports matching callbacks across multiple markets. On the third 2026-09-25 maintenance attempt, the foreground Runner Job stayed alive, SPARK login `0001` succeeded, fresh W1/W2 provenance was observed, exact `JNU2612` was resolved, and exactly one tick-detail raw measurement was captured. C2.2 correctly fail-closed on the observed `15:45:01` print; C2.3 was then built from that evidence. Read-only inspection at 17:30 Asia/Taipei shows the current-build safe-default recorder RUNNING with login `0001`, fresh quotes, health_reasons=[], runtime build `afd52f88a351541a`, and runtime measurement gate=false. **C2.3 typed runtime re-verification is still pending; do not start a second owner.**
...
- C2.3 offline validation: focused `59 passed`; broader `154 passed, 2 deselected`; full offline `1861 passed, 24 deselected, 132 warnings in 133.26s`, exit 0. Third-attempt foreground recorder graceful-shutdown PASS. A later current-build safe-default recorder invocation is now RUNNING with measurement gate=false.
...
- W8.2 CI runtime maintenance: GitHub latest releases are checkout `v7.0.1` and setup-python `v7.0.0`, both Node 24. Workflow now uses `actions/checkout@v7`, `actions/setup-python@v7`, and fixed `ubuntu-24.04` instead of `ubuntu-latest`, avoiding the observed Node-20 forced-runtime warning and the announced Ubuntu 26 migration. Static regression locks this contract. Targeted=`21 passed`; full offline=`1885 passed, 1 skipped, 24 deselected, 132 warnings in 166.54s`, exit 0; build unchanged `afd52f88a351541a`.
