...
2026-09-25 C2 controlled-measurement-path initial implementation is `ed3e63360faca93f6a5a6b0af89fc1ecb51702bf`; correlation hardening is `9496eb9afa65e647b5fceca86610247ff24e258e`; review follow-ups are `30d32754ff284a6b32370b236ed1fc40283304e9`, `0203e9becf0f0894c3d9f33cfdc2aece448db921`, `873e6bd9697efe1bc2c67d1767c708b23af2df10`; maintenance-control source is `4cca09117ffda0fb2ead12de737ffcaf8b92ad9c`; current source/config build is `351267947f7f468d`. The existing single-owner recorder now has an offline-verified control action capable of issuing one bounded OSE `GetStkTickDetail` request through that same owner, while tracked `tick_detail_measurements.enabled=false` remains the safe default. Maintenance-only enable is now a runtime start flag, and graceful shutdown is a control-inbox action that exits through the recorder's own close/flush path rather than requiring a hard kill.
...
Validation for the latest source: maintenance focused `41 passed, 2 deselected`; broader regression `106 passed, 2 deselected`; final offline profile `1857 passed, 24 deselected, 132 warnings in 136.18s`, exit 0, excluding only the live-owner-conflicting `test_single_instance_lock_releases_after_error`. Targeted changed-file secret scan = 0; diff check PASS. No dependency or license surface was added.

Live truth: the current recorder is actively writing W1/W2 `PER_FIELD_ONLY` provenance, so whole-recorder adoption is no longer pending. The process still started around 07:41 Asia/Taipei, before the current C2 measurement/maintenance build, so **C2_CONTROL_PATH_RUNTIME_ADOPTION_PENDING**. The user explicitly authorized the maintenance window on 2026-09-25, but at authorization time the clock was still before the required 15:45 JST controlled measurement window. An attempted console Ctrl+C could not attach to the hidden old process, so no hard kill was performed. The old owner remains running; no second login owner was created and no tick-detail measurement request was queued. Actual runtime timestamp evidence remains **NONE_YET**; eligible real DAILY terminal close remains **NONE_YET**; W3.2 ACTUAL_FORWARD_EVIDENCE remains **NONE_YET**.

Next bounded step: maintenance authorization is already granted. Do not ask again for the same window. Near/after the valid OSE 15:45 JST day close, use the graceful control-inbox shutdown where available, adopt current published build `351267947f7f468d` in the single recorder owner with runtime-only `-EnableTickDetailMeasurements`, verify `runtime_build_id` + fresh heartbeat + `tick_detail_measurements_runtime_enabled=true`, then queue exactly one exact-JNU measurement before 17:00 JST. After evidence capture, return the recorder to the tracked safe-default mode.
