...
Implementation commit `d6d443a1d18989e4826c81ed0fcff835f5295386` closes the dirty W1 contract-roll package without broker mutation. The recorder now re-resolves default subscriptions every 300 seconds using each venue's local calendar date instead of treating UTC rollover as exchange/session rollover; failed revalidation remains visible and is retried on the next interval. Default and dynamic subscription ownership are separate, so a contract can be default-routed while retaining dynamic intent, and later default removal does not accidentally unsubscribe that dynamic request. Partial refresh failures keep the already-completed provider/local union truthful. Add-before-remove continuity now fail-closes before a transient 2000-subscription cap breach rather than removing old coverage first.

The bundled `YSendOrder.py` vendor sample calls `SubscribeWatchlistAll` / `UnSubscribeWatchlistAll` with two arguments. Offline reflection of the installed `2.2026.0918.0` DLL shows a third `Lng` parameter that is optional with default `NORMAL`; the existing explicit `enumLangType.UTF8` call therefore remains valid and was intentionally preserved. Subscribe/unsubscribe continue to share the conservative 0.2-second throttle (5 calls/sec, below the documented 10 calls/sec ceiling).
...
W8.2 CI runtime maintenance removes the GitHub Actions Node-20 deprecation and `ubuntu-latest` migration risk. GitHub release API on 2026-09-26 reports latest checkout=`v7.0.1` and setup-python=`v7.0.0`; both action metadata use Node 24. CI now uses `actions/checkout@v7`, `actions/setup-python@v7`, and pins `runs-on: ubuntu-24.04` instead of allowing `ubuntu-latest` to drift to Ubuntu 26. A static regression prevents reintroduction of the old majors or floating runner. Targeted reconstruction=`21 passed`; full offline=`1885 passed, 1 skipped, 24 deselected, 132 warnings in 166.54s`, exit 0. Implementation commit=`ac40f88b377b09eca0d296e0232abc88f4bd13c2`; CI `36210103584` SUCCESS; PR #55 remains OPEN/MERGEABLE. Product build remains `afd52f88a351541a`.

W8.3 external-relocation preflight makes the remaining machine/external gates observable without pretending to close them. New `scripts/check_external_relocation_gates.ps1 -Json` is read-only: it does not install dependencies, write WinCred, import/export certificates, register COM, or log into a broker. It reports local WinCred profile configuration state, generic Windows certificate-store state, and the existing read-only legacy-COM `READY_FOR_AUTH` smoke while every new-machine/external gate remains `UNVERIFIED_EXTERNAL_GATE`. `check_yuanta_certificate.ps1` was corrected so a non-empty/unexpired CurrentUser certificate store can no longer be reported as a verified Yuanta certificate; identity and official-signature verification remain explicitly false pending the official Yuanta certificate-center check. WebCodex Runner also reproduced two real Chinese-path encoding failures in the W7.5/W7.6 dry-run contracts; `render_mcp_config.py` and both Scheduled Task dry-run scripts now force UTF-8 output. Targeted reconstruction=`23 passed`; full offline=`1887 passed, 1 skipped, 24 deselected, 132 warnings in 148.29s`, exit 0. Product build remains `afd52f88a351541a`.
...
Historical W2 handoff recorded recorder adoption as pending. That historical observation later became stale. The 2026-09-25 read-only inspection was the then-current truth: a same-build safe-default recorder was RUNNING with `PER_FIELD_ONLY` provenance, runtime build `afd52f88a351541a`, and tick-detail measurement runtime gate=false. The current 2026-09-26 truth is the W1 section above: disk build is newer and runtime adoption is pending. C2.3 typed runtime re-verification is separately pending. Do not infer process build from disk alone; use the recorder's own `runtime_build_id`.
