...
wins. Refresh with `scripts/agent_bootstrap.ps1`. Last updated: 2026-09-25 C2 first authorized maintenance attempt fail-closed + startup observability.
...
One current-build owner start was attempted with runtime-only measurement enabled. The launcher printed `YUANTA_LIVE_STARTING pid=9060`, but that process and any recorder child disappeared before a fresh status or log was written; the old status remained stale and both recorder stdout/stderr files were zero-length. Per the maintenance contract, **no broker login retry and no GetStkTickDetail request were made**. Subsequent non-login diagnostics passed: current build/OrderApiExposureGuard/credential presence/FunctionList/default-subscription resolution/SparkRuntime constructor and Start-Process CLI mechanics. Therefore the first live attempt is **FAIL_CLOSED_STARTUP_UNOBSERVED**; the exact old-code failure stage cannot be proven, only bounded to after successful offline preflight. **The recorder is currently NOT RUNNING.** Actual runtime timestamp evidence remains **NONE_YET**; eligible real DAILY terminal close remains **NONE_YET**; W3.2 ACTUAL_FORWARD_EVIDENCE remains **NONE_YET**.

Next bounded step is a **new maintenance start attempt**, not an automatic retry of the failed one. Use published build `ba7c0e1b9ca9d62c` so startup stage telemetry is active; verify fresh `STARTING/RUNNING/DEGRADED` status and only then consider one exact `JNU2612` measurement inside a valid 15:45–17:00 JST window. If startup returns `START_FAILED`, report `startup_stage/fatal_error/login_msg_code` and stop without another login retry. After any successful evidence capture, restore safe-default recorder mode.
