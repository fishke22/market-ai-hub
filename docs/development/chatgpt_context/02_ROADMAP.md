...
W0 與 W1 的核心根因修补已在程式完成，詳見 03；不用再照舊稽核重做。W1 仍有 runtime adoption、自動 reconnect/roll、crash durability 與長期壓測未驗。下一棒以受控交接 + W2 reader/replay 為範圍。下面是完整最終驗收目標，不能誤讀成全部已完成。W3 的到期/scope/as-of gate 必須先於 W4 fitting。
