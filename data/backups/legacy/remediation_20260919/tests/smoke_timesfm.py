...
def main() -> int:
    ...
    # 2. univariate quantile
    try:
        pred = adapter.predict(synthetic, horizon=7, quantiles=[0.1, 0.5, 0.9])
        ok = sorted(pred.keys()) == ["p10", "p50", "p90"] and pred["p10"] <= pred["p50"] <= pred["p90"]
        results["univariate_quantile"] = "PASS" if ok else f"FAIL: {pred}"
        print("[2] univariate quantile:", "PASS" if ok else "FAIL", pred)
    except Exception as e:
    ...
    try:
        ...
        results["past_only_covariates"] = "PASS" if sorted(pred2.keys()) == ["p10", "p50", "p90"] else f"FAIL: {pred2}"
...
def _save(results: dict) -> None:
    ...
    out = r"D:\MARKET_AI_HUB\reports\timesfm_smoke.json"
